...
{
    class CorrectScoreBetPlacer : BetPlacer
    {

        public CorrectScoreBetPlacer(TRADETYPE tradeType, double odd, String match, long marketId, long selectionId, TTRWatcher watcher,
        ...
        public override bool placeBet()
        {
            ...
            else if(_config.FastBetFixedAmount || (!_config.FastBetFixedAmount && !_config.RelativeBetAmount && !_config.FastBetCalculatedAmount))
            {
                bRet = placeBetAbsolut(out moneyToBet);
            }
            else if (_config.FastBetCalculatedAmount)
            {
                bRet = placeBetTarget(out moneyToBet);
            }
            if (bRet)
                return placeBet(moneyToBet);

            return bRet;

        }

        private bool placeBet(double moneyToBet)
        {
            ...
            try
            {
                ...
                if (this._tradeMode == TRADEMODE.BACK)
                    try
                    {
            ...
            catch (SXALBetInProgressException bipe)
            {
                ...
                if (muBets != null)
                {
                    foreach (SXALMUBet muBet in muBets)
                    {
                        ...
                        {
                            if (_watcher[_match].ContainsKey(this._tradeType))
                            {
                                // Uns interessieren hier nur neue Wetten                                
                                if (((CorrectScoreTrade)_watcher[_match][this._tradeType]).Back.Bets.ContainsKey(muBet.BetId))
                                    continue;
                            }
                        }
            ...
            if (bet != null)
            {
                ...
                else
                {
                    ...
                    if (bet.BetStatus == SXALBetStatusEnum.U)
                    {
                        ...
                        //Pseudoentlosschleife: Abfangen, wenn das Ergebnis eines Stornierungsvesuchts
                        //immer noch eine unmatched Wette ist.
                        while (true)
                        {
                            try
                            {
