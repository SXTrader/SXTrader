...
{
    class Scoreline00BetPlacer : BetPlacer
    {
        public Scoreline00BetPlacer(TRADETYPE tradeType, double odd, String match, long marketId, long selectionId, TTRWatcher watcher,
        ...
        public override bool placeBet()
        {
            ...
            log("Place a Scoreline 0 - 0 Bet for Trade In");
            ...
            SXALMarket market = SXALSoccerMarketManager.Instance.getCSMarketByMatch(_match);
            _marketId = market.Id;
            ...
            catch (SXALBetInProgressException bipe)
            {
                ...
                if (muBets != null)
                {
                    ...
                    {
                        if (muBet.SelectionId != (int)SXALKom.Instance.getSelectionId(SXALSelectionIdEnum.CSZEROZERO, muBet.MarketId))
                            continue;
                        ...
                        if(_watcher.ContainsKey(_match))
                        {
                            if (_watcher[_match].ContainsKey(this._tradeType))
                            {
                                ...
                                if(((ScorelineTrade00)_watcher[_match][this._tradeType]).Back.Bets.ContainsKey(muBet.BetId))                                
                                    continue;
                            }
                        }
            ...
            if (bet != null)
            {
                ...
                else
                {
                    ...
                    if (bet.BetStatus == SXALBetStatusEnum.M)
                    {
                        ...
                        _watcher.setNewBet(TRADETYPE.SCORELINE00, bet, _config, _score);
                        return true;
                    }
