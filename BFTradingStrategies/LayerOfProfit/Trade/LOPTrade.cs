...
{
    class LOPTradeList : List<LOPTrade> { }
    sealed class LOPTrade : IDisposable, IBFTSCommon
    {
        public enum TRADESTATE { STARTED, COMPLETED, TRADING, NONRUNNER, NOTTRADING };

        private LOPConfigurationRW _settings;
        ...
        #region Events
        public delegate void LOPTradeSettledHandler(object sender, LOPTradeSettledEventArgs e);
        public event LOPTradeSettledHandler TradeSettledEvent;
        #endregion
        ...
        public double MatchedAmount
        {
            get
            {
                ...
                {
                    money += _matchedBets.Values[i].MatchedSize;                
                }
        ...
        public double UnmatchedAmount
        {
            get
            {
                ...
                {
                    money += _openBets.Values[i].RemainingSize;
                }
                return Math.Round(money, 2);
            }
        }
        ...
        public double Risk
        {
            get
            {
                ...
                for (int i = 0; i < _matchedBets.Values.Count; i++)
                {
                    risk += (_matchedBets.Values[i].MatchedSize * _matchedBets.Values[i].AvgPrice - _matchedBets.Values[i].MatchedSize);
                }
        ...
        public double PotentialRisk
        {
            get
            {
                ...
                for (int i = 0; i < _openBets.Values.Count; i++)
                {
                    risk += (_openBets.Values[i].RemainingSize * _openBets.Values[i].Price - _openBets.Values[i].RemainingSize);
                }
        ...
        public double AvgPrice
        {
            get
            {
                ...
                if (_matchedBets.Values.Count > 0)
                {
                    for (int i = 0; i < _matchedBets.Values.Count; i++)
                    {
                        avgPrice += _matchedBets.Values[i].AvgPrice;
                    }
        ...
        private LOPTrade()
        {
            ...
            _settings = new LOPConfigurationRW();

            SXMinutePulse.Instance.Pulse += Instance_Pulse;
            SX5MinutePulse.Instance.Pulse += Instance_5Pulse;
        }

        private LOPTrade(SXALMarket market, long selectionId, string horseName)
        {
            _state = TRADESTATE.STARTED;
            ...
            if (odds < 1.01)
            {
                ...
                throw new LOPInvalidOddsException(_horse, _race, _dts);
            }
        ...
        private LOPTrade(SXALBet[] bets, LOPConfigurationRW settings)
        {
            try
            {                
                _state = TRADESTATE.STARTED;
                ...
                foreach (SXALBet bet in bets)
                {
                    ...
                    if (_selectionId != 0 && _selectionId != bet.SelectionId)
                    {
                        ...
                        //Ausnahme auslösen
                        throw new LOPSelectionIDMismatchException(_selectionId, bet.SelectionId);
                    }
            ...
            lock(_syncLock)
            {
                try
                {
                    ...
                    if (marketLite.MarketStatus == SXALMarketStatusEnum.SETTLED || marketLite.MarketStatus == SXALMarketStatusEnum.CLOSED)
                    {
                        ...
                        {
                            TradeSettledEvent(this, new LOPTradeSettledEventArgs());
                        }
            ...
            lock(_syncLock)
            {
                try
                {
                    ...
                    if (_settings.UseFinalTime && _dts.Subtract(DateTime.Now).TotalMinutes <= _settings.FinalTime)
                    {
                        ...
                        for (int i = 0; i < _openBets.Values.Count; i++)                        
                        {
                            if (_openBets.Values[i].Price == _settings.MaximumLayOdd)
                            {
                                log(String.Format("Bet {0} has alread maximul odds of {0}. Bet will not be canceled", _openBets.Values[i].BetId, _settings.MaximumLayOdd));
                                continue;
                            }
                            log(String.Format("Cancel bet {0}", _openBets.Values[i].BetId));
        ...
        {            
            ...
            if (_settings.UseStepTrading && runnerPrice.BestPricesToBack != null && runnerPrice.BestPricesToBack.Length >= 2 &&
                runnerPrice.BestPricesToBack[0].Price == bet.Price && runnerPrice.BestPricesToLay[0].Stake <= bet.RemainingSize)
            {
            ...
            if (_settings.UseStepTrading && runnerPrice.BestPricesToBack != null && runnerPrice.BestPricesToBack.Length > 0 &&
                runnerPrice.BestPricesToBack[0].Price > bet.Price && runnerPrice.BestPricesToBack[0].Price <= _settings.MaximumLayOdd)
            {
        ...
        public static LOPTrade CreateTrade(SXALBet[] bets, LOPConfigurationRW settings)
        {
            return new LOPTrade(bets, settings);
        }

        public static LOPTrade CreateTrade(SXALMarket market, long selectionId, string horseName)
        {
            return new LOPTrade(market, selectionId, horseName);
        }
        ...
        private bool isHorseNonRunner()
        {
            ...
            throw new LOPHorseNotFoundExcpetion(_horse, _race, _market.Id);
        }
        ...
        private bool checkNonRunners()
        {
            ...
            if (_settings.NonStarterObservation)
            {
                ...
                if (selections.Length <= LOPConfigurationRW.FIELD1SIZE && countNR >= _settings.NonRunnerCount1)
                {
                ...
                if (selections.Length <= LOPConfigurationRW.FIELD2SIZE && countNR >= _settings.NonRunnerCount2)
                {
                ...
                if (selections.Length <= LOPConfigurationRW.FIELD3SIZE && countNR >= _settings.NonRunnerCount3)
                {
                ...
                if (selections.Length <= LOPConfigurationRW.FIELD4SIZE && countNR >= _settings.NonRunnerCount4)
                {
                ...
                if (selections.Length > LOPConfigurationRW.FIELD4SIZE && countNR >= _settings.NonRunnerCount5)
                {
        ...
        private double getOdds(SXALMarketPrices prices)
        {
            ...
            if (runnerPrice == null)
            {
                ...
                throw new LOPNoRunnerPricesException(_race, _horse, _dts);
            }
            ...
            if (_settings.UseFinalTime && _dts.Subtract(DateTime.Now).TotalMinutes <= _settings.FinalTime)
            {
                ...
                {
                    log(String.Format("There are no lay odds. Setting Odds to Maximum Odds {0}", _settings.MaximumLayOdd));
                    odds = _settings.MaximumLayOdd;
                }
                //Layquoten vorhanden
                else if (runnerPrice.BestPricesToLay[0].Price > _settings.MaximumLayOdd)
                {
                    log(String.Format("Lay Odds found but Odds {0} are bigger than Maximum Odds {1}. Setting Odds to Maximum Odds {1}",
                        runnerPrice.BestPricesToLay[0].Price, _settings.MaximumLayOdd));
                    odds = _settings.MaximumLayOdd;
                }
                else
                {
            ...
            {
                log(String.Format("Neither Back nor Lay Odds given. Setting Odds to half Maximum Odds {0}", _settings.MaximumLayOdd / 2));
                odds = _settings.MaximumLayOdd / 2;
            }
            ...
            else if(runnerPrice.BestPricesToLay == null || runnerPrice.BestPricesToLay.Length == 0)
            {
                ...
                if (runnerPrice.BestPricesToBack[0].Price >= _settings.MaximumLayOdd)
                {
                    log(String.Format("Back Odds greater than Maximum Odds. Setting Odds to Maximum Odds", _settings.MaximumLayOdd));
                    odds = _settings.MaximumLayOdd;
                }
                else
                {
            ...
            else if (runnerPrice.BestPricesToBack == null || runnerPrice.BestPricesToBack.Length == 0)
            {
                ...
                if (runnerPrice.BestPricesToLay[0].Price >= _settings.MaximumLayOdd)
                {
                    log(String.Format("Lay Odds greater than Maximum Odds. Setting Odds to Maximum Odds", _settings.MaximumLayOdd));
                    odds = _settings.MaximumLayOdd;
                }
                else
                {
            ...
            else
            {
                ...
                if (_settings.UseStepTrading && diffTick >= _settings.StepTradingGap && runnerPrice.BestPricesToBack[0].Price < _settings.MaximumLayOdd)
                {
                    log(String.Format("Gap Trading allowed and Gap between Back and Lay is {0}. Customized Gap is {1}. Back Odds smaller than Maximum Odds {2}", 
                        diffTick, _settings.StepTradingGap, _settings.MaximumLayOdd ));
                    decimal incr = SXALKom.Instance.getValidOddIncrement((decimal)runnerPrice.BestPricesToBack[0].Price);
                    odds = (double)(((decimal)runnerPrice.BestPricesToBack[0].Price) + incr);
                    odds = (double)SXALKom.Instance.validateOdd((decimal)odds);
                    log(String.Format("Back Odds smaller than Maximum Odds. Setting Odds to Back Odds + 1 Tick {0}", odds));
                ...
                else
                {
                    ...
                    {
                        odds = _settings.MaximumLayOdd;
                        log(String.Format("Lay Odds greater or equal than Maximum Odds. Setting Odds to Maximum Odds", _settings.MaximumLayOdd));
                    }
                    else
                    {
        ...
        public void log(string message)
        {
            try
            {
                ...
                TradeLog.Instance.writeLog(id, "LayerOfProfit", "Trader", String.Format("ID {0}: {1}", id, message));
            }
            catch { }
        }
        ...
        public void logBetAmount(string message)
        {
            ...
            {
                String id = String.Format("{0}_{1}_{2}", this._race, this._dts.ToString(), this._horse);
                TradeLog.Instance.writeBetAmountLog(id, "Layer Of Profit", "Trader", String.Format("ID {0}: {1}", id, message));
            }
            catch { }
        }
        ...
        public void logBetDetails(SXALBet b)
        {
            ...
            {
                String id = String.Format("{0}_{1}_{2}", this._race, this._dts.ToString(), this._horse);
                TradeLog.Instance.writeLog(id, "Layer Of Profit", "Trader", String.Format("Bet ID: {0};Bet Type: {1}; Bet Status: {2}", b.BetId, b.BetType, b.BetStatus));

                TradeLog.Instance.writeLog(id, "Layer Of Profit", "Trader", String.Format("Price: {0}; Average Price: {1}", b.Price, b.AvgPrice));
                TradeLog.Instance.writeLog(id, "Layer Of Profit", "Trader", String.Format("Requested Size: {2}; Matched Size: {0}; Remaining Size: {1}", b.MatchedSize, b.RemainingSize, b.RequestedSize));
                TradeLog.Instance.writeLog(id, "Layer Of Profit", "Trader", String.Format("Placed Date: {0}; Matched Date: {1}; Settled Date: {2}; Cancelled Date: {3}", b.PlacedDate, b.MatchedDate, b.SettledDate, b.CancelledDate));
                TradeLog.Instance.writeLog(id, "Layer Of Profit", "Trader", String.Format("Profit & Loss: {0}", b.ProfitAndLoss));

                if (b.Matches == null || b.Matches.Length == 0)
                {
                    TradeLog.Instance.writeLog(id, "Layer Of Profit", "Trader", "No Detailed Matches");
                }
                else
                {
                    foreach (SXALMatch m in b.Matches)
                    {
                        TradeLog.Instance.writeLog(id, "Layer Of Profit", "Trader", String.Format("Size Match: {0}", m.SizeMatched));
                    }
                }
...
#region IDisposable
        ...
        void Dispose(bool disposing)
        {
            if (!_disposed)
            {
                DebugWriter.Instance.WriteMessage("LOPTrade", "Disposing");
                if (disposing)
                {
                    if (_settings != null)
                    {
    ...
    class LOPBelowMinStakeException : Exception
    {
        private LOPBelowMinStakeException() : base() { }

        public LOPBelowMinStakeException(String race, String horse, DateTime dts, double amount, String sportExchange)
            : base(String.Format(LayerOfProfit.strBelowMinStake, horse, race, dts, amount, sportExchange))
        {
        }
    }

    class LOPNoRunnerPricesException : Exception
    {
        private LOPNoRunnerPricesException() : base() { }

        public LOPNoRunnerPricesException(String race, String horse, DateTime dts)
            : base(String.Format(LayerOfProfit.strNoPriceInformation, horse, race, dts))
        {
        }
    }

    class LOPSelectionIDMismatchException : Exception
    {
        private LOPSelectionIDMismatchException()
            : base()
        {
        }

        public LOPSelectionIDMismatchException(long selectionId1, long selectionId2) : base(String.Format(LayerOfProfit.strSelectionIdMismatch, selectionId1, selectionId2))
        {
            
        }
    }

    class LOPHorseNotFoundExcpetion : Exception
    {
        private LOPHorseNotFoundExcpetion() : base() { }


        public LOPHorseNotFoundExcpetion(String horse, String race, long marketId) : base(String.Format(LayerOfProfit.strHorseNotFound, horse, race, marketId)) { }
    }

    class LOPInvalidOddsException : Exception
    {
        private LOPInvalidOddsException() : base() { }

        public LOPInvalidOddsException(String horse, String race, DateTime dts) : base(String.Format(LayerOfProfit.strInvalidOdds, horse, race, dts)) { }
    }

    class LOPTradeSettledEventArgs : EventArgs
    {        
    }
}
