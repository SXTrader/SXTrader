...
{
    class TLLWatcher : IBFTSCommon, IDisposable
    {
        /// <summary>
        /// Singleton Instanz
        /// </summary>
        private static volatile TLLWatcher _instance;
        /// <summary>
        ...
        private TLLMails _lopMails;
        private TLLTradeList _trades;
        private TLLTipp _lastTipp;
        private bool _disposed = false;
        private System.Timers.Timer _checkTimer;
        private object _syncMailCheck = "syncMailCheck";

        public delegate void TLLNoSelectionDayHandler(object sender, TLLNoSelectionDayEventArgs e);
        public event TLLNoSelectionDayHandler NoSelectionDayEvent;

        public delegate void TLLNoTippFoundHandler(object sender, TLLNoTippFoundEventArgs e);
        public event TLLNoTippFoundHandler NoTippFoundEvent;

        public delegate void TLLTippPlacedHandler(object sender, TLLTippAddedEventArgs e);
        public event TLLTippPlacedHandler TippPlacedEvent;

        public delegate void TLLTippSettledHandler(object sender, TLLSettledEventArgs e);
        public event TLLTippSettledHandler TippSettledEvent;

        public delegate void TLLTippUpdateHandler(object sender, TLLTippAddedEventArgs e);
        public event TLLTippUpdateHandler TippUpdateEvent;

        public static TLLWatcher Instance
        {
            get
            {
                if (_instance == null)
        ...
        private TLLWatcher()
        {
            ...
            _lopMails = new TLLMails();
            _trades = new TLLTradeList();
        ...
        private void infoUpdater(Object stateInfo)
        {
            for (int i = 0; i < _trades.Count; i++)
            {
                ...
                {
                    TippUpdateEvent(this, new  TLLTippAddedEventArgs(_trades[i].EventDate, _trades[i].Race, _trades[i].Horse, _trades[i].Currency, _trades[i].MatchedAmount,
        ...
        private void initialLoadFromLazerus()
        {
            try
            {
                ...
                LazerusList list = Lazerus.Instance.getEntriesByModuleName(SXALKom.Instance.getExchangeName(), TheLowLay.strName);
                ...
                foreach (LazerusEntry entry in list)
                {
                    foreach (LazerusEntry.LazerusNode node in entry.ExistTrades)
                    {
                        ...
                        foreach (long betId in node.BetIds)
                        {
                            ...
                            switch (bet.BetStatus)
                            {
                                ...
                                case SXALBetStatusEnum.V:
                                    ...
                                    Lazerus.Instance.removeBetFromTrade(SXALKom.Instance.getExchangeName(), TheLowLay.strName, Lazerus.ENTRYMODES.EXISTMODE, bet.BetId);
                                    break;
                            }
                        ...
                        if (listBet.Count > 0)
                        {
                            ...
                            TLLTrade trade = TLLTrade.CreateTrade(listBet.ToArray(), new TLLConfigurationRW(node.Settings));
                            if (trade != null)
                            {
                                ...
                                {
                                    TippPlacedEvent(this, new TLLTippAddedEventArgs(trade.EventDate, trade.Race, trade.Horse, trade.Currency, trade.MatchedAmount,
        ...
        void trade_TradeSettledEvent(object sender, TLLTradeSettledEventArgs e)
        {
            for (int i = 0; i < _trades.Count; i++)
            {
                if (_trades[i] == sender)
                {
                    ...
                    {
                        TippSettledEvent(this, new TLLSettledEventArgs(_trades[i].EventDate, _trades[i].Race, _trades[i].Horse, TLLTrade.TRADESTATE.COMPLETED));
                    }

                    _lastTipp = new TLLTipp();
        ...
        private async void _checkTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            //lock (_syncMailCheck)
            {
                try
                {
                    ...
                    TLLTipp tipp = await _lopMails.getLOPTipp();
                    ...
                    if (tipp != null)
                    {
                        if (tipp.NoSelectionDay)
                        {
                            if (NoSelectionDayEvent != null)
                                NoSelectionDayEvent(this, new TLLNoSelectionDayEventArgs());
                            return;
                        }
                        ...
                        if (tipp.NoTippFound)
                        {
                            if (NoTippFoundEvent != null)
                                NoTippFoundEvent(this, new TLLNoTippFoundEventArgs());
                            return;
                        }


                        //Gibt es schon einen Trade zu dieser Kombination?
                        foreach (TLLTrade trade in _trades)
                        {
                        ...
                        if (tipp.EventDate.Subtract(DateTime.Now).TotalMilliseconds < 0)
                        {
                            ...
                            {
                                MessageEvent(this, new SXWMessageEventArgs(DateTime.Now, TheLowLay.strWatcher, String.Format(TheLowLay.strEventInPast, tipp.Race, tipp.Horse, tipp.EventDate)
                                    , TheLowLay.strName));
                            }
                            return;
                        }
                        ...
                        if (m == null)
                        {
                            if (MessageEvent != null)
                            {
                                MessageEvent(this, new SXWMessageEventArgs(DateTime.Now, TheLowLay.strWatcher, String.Format(TheLowLay.strNoMarketFound, tipp.Race, tipp.Horse, tipp.EventDate),
                                    TheLowLay.strName));
                            }
                            return;
                        }
                        ...
                        if (selectionId == -1)
                        {
                            if (MessageEvent != null)
                            {
                                MessageEvent(this, new SXWMessageEventArgs(DateTime.Now, TheLowLay.strWatcher, String.Format(TheLowLay.strHorseNotFound, tipp.Horse, tipp.Race, m.Id, tipp.EventDate),
                                    TheLowLay.strName));
                            }
                            return;
                        }
                        ...
                        //Erstelle Trade
                        TLLTrade lopTrade = TLLTrade.CreateTrade(m, selectionId, tipp.Horse, tipp.MaxOdds);
                        if (lopTrade != null)
                        {
                            ...
                            if (TippPlacedEvent != null)
                            {
                                TippPlacedEvent(this, new TLLTippAddedEventArgs(lopTrade.EventDate, lopTrade.Race, lopTrade.Horse, lopTrade.Currency, lopTrade.MatchedAmount,
                                    lopTrade.UnmatchedAmount, lopTrade.Risk, lopTrade.PotentialRisk, lopTrade.AvgPrice, lopTrade.TradeState));
                            }

                            if (MessageEvent != null)
                            {
                                MessageEvent(this, new SXWMessageEventArgs(DateTime.Now, TheLowLay.strWatcher, String.Format(TheLowLay.strTradeAdded, tipp.Race, tipp.Horse, tipp.EventDate),
                                    TheLowLay.strName));
                            }
                        }
                    }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposed)
            {
                DebugWriter.Instance.WriteMessage("TLLWatcher", "Disposing");
                if (disposing)
                {
                    if (_checkTimer != null)
                        _checkTimer.Dispose();
    ...
    class TLLNoSelectionDayEventArgs : EventArgs { }

    class TLLNoTippFoundEventArgs : EventArgs { }

    class TLLSettledEventArgs : EventArgs
    {
        private DateTime _eventDate;
        private String _race;
        private String _horse;
        private TLLTrade.TRADESTATE _state;
        ...
        public TLLTrade.TRADESTATE TradeState
        {
            get { return _state; }
        }

        public TLLSettledEventArgs(DateTime eventDate, String race, String horse, TLLTrade.TRADESTATE state)
        {
    ...
    class TLLTippAddedEventArgs : EventArgs
    {
        private DateTime _eventDate;
        private String _race;
        private String _horse;
        ...
        public TLLTippAddedEventArgs(DateTime eventDate, String race, String horse, String currency, double matchedAmount, double openAmount, double risk,
             double potentialRisk, double avgOdds, TLLTrade.TRADESTATE state)
        {
            _eventDate = eventDate;
            _race = race;
            _horse = horse;
    ...
    class TLLTippUpdateEventArgs : EventArgs
    {
        private DateTime _eventDate;
        private String _race;
        private String _horse;
        ...
        public TLLTippUpdateEventArgs(DateTime eventDate, String race, String horse, String currency, double matchedAmount, double openAmount, double risk,
             double potentialRisk, double avgOdds, TLLTrade.TRADESTATE state)
        {
            _eventDate = eventDate;
            _race = race;
